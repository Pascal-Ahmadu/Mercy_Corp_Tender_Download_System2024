...
export const AuthProvider = ({ children }) => {
  ...
  // Login function with session token generation and fetching user details
const login = async (email, password) => {
  ...
  if (data && bcrypt.compareSync(password, data.password)) {
    ...
    const userDetails = await fetchUserDetails(email); // Assuming fetchUserDetails is defined
    if (userDetails) {
      // Add user details to the user object
      user.name = userDetails.name; // Assuming userDetails contains a 'name' field
