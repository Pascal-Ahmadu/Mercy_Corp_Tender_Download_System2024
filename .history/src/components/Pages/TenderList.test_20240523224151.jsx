...
test('displays tenders', async () => {
  ...
  await waitFor(() => {
    expect(screen.getByText((content, element) => content.includes('Title: Tender 1'))).not.toBeNull();
    expect(screen.getByText((content, element) => content.includes('Title: Tender 2'))).not.toBeNull();
  });
});
...
test('handles search input correctly', async () => {
  ...
  await waitFor(() => {
    ...
    expect(screen.getByText((content, element) => content.includes('Title: Tender 1'))).not.toBeNull();
    expect(screen.queryByText((content, element) => content.includes('Title: Tender 2'))).toBeNull();
  });
});
