...
const UploadBid = () => {
  ...
  useEffect(() => {
    const fetchUserBids = async () => {
      ...
      if (error) {
        ...
        setUserBid(data[0]); // Assuming there is only one user bid, otherwise, adjust as needed
      }
    };
  ...
  const handleDelete = async (bidFileName) => {
    setLoading(true);
    const { error } = await supabase
      .from('bids')
      .delete()
      .eq('uploaded_by', user.email)
      .eq('file_name', bidFileName);

    if (error) {
      console.error('Error deleting bid:', error.message);
    } else {
      setUserBids((prevBids) => prevBids.filter((bid) => bid.file_name !== bidFileName));
    }
    setLoading(false);
  };
  return (
