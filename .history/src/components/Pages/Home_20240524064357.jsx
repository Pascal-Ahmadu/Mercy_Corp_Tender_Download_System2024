  ...
  useEffect(() => {
    ...
    const fetchTenderStats = async () => {
      try {
        ...
        const { data: availableTenders, error: availableError } = await supabase
          .from('tenders')
          .select('id')
          .gt('submission_deadline', today);
        if (availableError) throw availableError;

        const availableTendersCount = availableTenders ? availableTenders.length : 0;

        const { data: openedTenders, error: openedError } = await supabase
          .from('tenders')
          .select('id')
          .eq('status', 'opened');
        if (openedError) throw openedError;

        const openedTendersCount = openedTenders ? openedTenders.length : 0;

        const { data: availableBids, error: bidsError } = await supabase
          .from('bids')
          .select('id');
        if (bidsError) throw bidsError;

        const availableBidsCount = availableBids ? availableBids.length : 0;
